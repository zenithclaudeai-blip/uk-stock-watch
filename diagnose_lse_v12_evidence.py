...
def make_request(path, parameters, component_id, timeout=15):
    ...
    try:
        ...
        result.update({
            ...
            "finalUrl": resp.url,
            "redirected": resp.url != LSE_COMPONENTS_REFRESH_URL,
        })
...
def run_heatmap():
    ...
    with open("lse_v12_heatmap_response.json", "w") as f:
        f.write(result.get("bodyText", ""))
    print("Full body saved to lse_v12_heatmap_response.json")
    ...
    if result.get("bodyText"):
        try:
            ...
            with open("lse_v12_heatmap_shape.json", "w") as f:
    ...
    with open("lse_v12_heatmap_result.json", "w") as f:
...
def run_news_isolated(attempt_num):
    ...
    print(f"Body bytes: {result.get('bodyBytes')}")
    print(f"Body SHA-256: {result.get('bodySha256')}")
    print(f"Final URL: {result.get('finalUrl')} (redirected: {result.get('redirected')})")
    if result.get("error"):
        print(f"ERROR: {result['error']}")
    # Printed directly in the log (not just saved to a file) since the
    # body is tiny and this is the exact piece of evidence needed next -
    # no artifact download required to see it.
    print(f"RAW BODY (repr): {result.get('bodyText', '')!r}")
    print(f"Request headers sent: {dict(result.get('requestHeaders', {}))}")
    print(f"Response headers received: {result.get('responseHeaders', {})}")
    ...
    if result.get("bodyText"):
        with open(f"lse_v12_news_attempt_{attempt_num}_response.json", "w") as f:
    ...
    results_file = "lse_v12_news_all_attempts.json"
    try:
